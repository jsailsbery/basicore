...
class Grep:
    """
    A utility class for searching and matching patterns in files within a source_dir and its subdirectories.
    This class provides methods for searching files based on a file name pattern, searching for a text
    pattern within files, and combining both operations to grep all files in a source_dir.
    """

    @classmethod
    def search_files(cls, directory: str, pattern: str, case_sensitive: bool = False) -> Iterator[str]:
        """
        Search for files matching a pattern in a source_dir and its subdirectories.

        :param directory: The source_dir to start the search from.
        ...
        if not os.path.isdir(directory):
            logger.error(f"Provided path is not a source_dir: {directory}")
            return iter([])
                ...
                with open(file_path, "r", errors='ignore') as file:
                    for line_number, line in enumerate(file, 1):
                        ...
                        if regex.search(line):
                            lines.append(f"{file_path}:{line_number}:{line}")
    ...
    def grep_all_files(cls, pattern: str, directory: str) -> List[str]:
        """
        Search for a pattern in all files within a source_dir and its subdirectories.

        This method combines the functionality of search_files and grep methods to search for a
        text pattern in all files within the specified source_dir and its subdirectories.

        :param pattern: The pattern to search for.
        :param directory: The source_dir to start the search from.
        :return: A list of lines containing the pattern from all matching files.
        """
        all_files = Grep.search_files(directory, "*")
